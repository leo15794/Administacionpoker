...
// Un supervisor puede tener % configurado sobre el rake semanal de un agente que él refirió
// (distinto de "agente a cargo" vía agents.supervisor) — se auto-acredita en cada cierre
// semanal (ver src/repo/closings.ts) como un saldo separado, nunca mezclado con la
// liquidación propia del agente referido.
// ============================================================

catalogRouter.get("/agents/:id/referidos", requireAuth, requireAdmin, async (req, res) => {
  const r = await pool.query(
    `SELECT r.*, a.name as agente_referido_name
     FROM supervisor_referidos r JOIN agents a ON a.id = r.agente_referido_id
     WHERE r.supervisor_agent_id = $1 AND r.active = true
     ORDER BY a.name`,
    [req.params.id]
  );
  res.json(r.rows);
...
catalogRouter.post("/agents/:id/referidos", requireAuth, requireAdmin, async (req, res) => {
  const parsed = crearReferidoSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() });
  const { agenteReferidoId, porcentaje } = parsed.data;
  if (agenteReferidoId === req.params.id) {
    return res.status(400).json({ error: "Un supervisor no puede referirse a sí mismo." });
  }
  try {
    const id = newId("refsup");
    const r = await pool.query(
      `INSERT INTO supervisor_referidos (id, supervisor_agent_id, agente_referido_id, porcentaje)
...
catalogRouter.patch("/referidos/:refId", requireAuth, requireAdmin, async (req, res) => {
